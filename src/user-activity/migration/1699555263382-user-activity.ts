...
export class UserActivity1699555263382 implements MigrationInterface {
  name = 'UserActivity1699555263382';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`user_activity\` (\`ID\` varchar(36) NOT NULL,\`created_at\` bigint NOT NULL, \`updated_at\` bigint NULL, \`deleted_at\` bigint NULL, \`user_activity_action\` enum ('Download', 'Watch', 'Rate', 'Subscribe', 'Comment', 'Like') NOT NULL, \`user_activity_status\` enum ('Pass', 'Fail') NOT NULL, \`user_activity_content_id\` varchar(255) NOT NULL, \`user_id\` varchar(36) NULL, PRIMARY KEY (\`ID\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
