...
export class MovieCrew1703905047489 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`movie_crew\` ADD CONSTRAINT \`FK_e11d125b8be76ef5893e7c08b15\` FOREIGN KEY (\`movie_id\`) REFERENCES \`movie\`(\`ID\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`movie_crew\` ADD CONSTRAINT \`FK_d4a490266a0ed67f49c352baf30\` FOREIGN KEY (\`crew_id\`) REFERENCES \`crew\`(\`ID\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    }
