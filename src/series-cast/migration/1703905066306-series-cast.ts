...
export class SeriesCast1703905066306 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`series_cast\` ADD CONSTRAINT \`FK_0df029204e4113011c5a7ee3827\` FOREIGN KEY (\`series_id\`) REFERENCES \`series\`(\`ID\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`series_cast\` ADD CONSTRAINT \`FK_18b1b88a5598dff32b53c9b6b2c\` FOREIGN KEY (\`cast_id\`) REFERENCES \`cast\`(\`ID\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }
