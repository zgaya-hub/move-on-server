...
export class ExternalLink1699555284784 implements MigrationInterface {
  name = 'ExternalLink1699555284784';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`external_link\` ADD CONSTRAINT \`FK_7c1457beaba3b923bce850c73f1\` FOREIGN KEY (\`episode_id\`) REFERENCES \`episode\`(\`ID\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    await queryRunner.query(`ALTER TABLE \`external_link\` ADD CONSTRAINT \`FK_56dee72a4d9abda8106e96a8816\` FOREIGN KEY (\`series_id\`) REFERENCES \`series\`(\`ID\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    await queryRunner.query(`ALTER TABLE \`external_link\` ADD CONSTRAINT \`FK_0e83d4b713a82cf5826bdf85853\` FOREIGN KEY (\`movie_id\`) REFERENCES \`movie\`(\`ID\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
